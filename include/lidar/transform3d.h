...
public:
    ...
    // Transform By delta value (Add/Scale)
    ...
    void rotate(float angle, const QVector3D &axis);  // in degrees
    void rotate(float angle, float ax, float ay, float az);  // in degrees
    void grow(const QVector3D &ds);
    void grow(float dx, float dy, float dz);
    void grow(float factor);

    // Transform To absolute value (Setters) in degree
    ...
    const QVector3D& getScaleVector() const;
    const QQuaternion& getRotationQuaternion() const;
    const QMatrix4x4& getObject2WorldMatrix();
    const QMatrix4x4& getWorld2ObjectMatrix();
...
private:
    ...
    QMatrix4x4 m_object2world;
    QMatrix4x4 m_world2object;
...
inline const QVector3D& Transform3D::getScaleVector() const { return m_scale; }
inline const QQuaternion& Transform3D::getRotationQuaternion() const { return m_rotation; }
